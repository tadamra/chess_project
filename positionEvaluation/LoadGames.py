...
def generate_data(filename, number_of_games_to_analyze=np.inf, number_of_positions=np.inf):
    ...
    while (game := chess.pgn.read_game(
            pgn)) is not None or counter < number_of_games_to_analyze and position_counter < number_of_games_to_analyze:
        counter += 1
        board = game.board()
        node = game
        for move in game.mainline_moves():
            ...
            if node.eval() is None:
                ...
                if not board.board_fen() in positions_added:
                    positions_added.add(board.board_fen())
...
if __name__ == '__main__':
    ...
    generate_data("lichess_db_standard_rated_2016-03.pgn", 20000000, number_of_positions=2000000)
